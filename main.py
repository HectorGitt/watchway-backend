...
async def create_user(
    ...
    verification_token = auth.create_access_token(
        data={"sub": user.email}, expires_delta=auth.timedelta(hours=24)
    )
...
def login_google(data: schemas.GoogleLogin, db: Session = Depends(get_db)):
    try:
        ...
        if not user:
            ...
            user = models.User(
                ...
                hashed_password="google_sso_no_password",  # Dummy password
            )
            db.add(user)
            db.commit()
            db.refresh(user)

        # Generate standard Watchway access token
        access_token_expires = auth.timedelta(minutes=auth.ACCESS_TOKEN_EXPIRE_MINUTES)
        access_token = auth.create_access_token(
        ...
        # Invalid token
        print(f"DEBUG: Google token invalid - {e}")
...
def verify_report(
    ...
    if report.reporter_id == current_user.id:
        raise HTTPException(
            status_code=400,
            detail="You cannot verify your own report. A different Citizen must confirm this from their location",
        )
    ...
    if x_device_id and report.device_id and x_device_id == report.device_id:
        raise HTTPException(
            status_code=400,
            detail="Security Alert: You cannot verify a report from the same device used to submit it.",
        )

    # 3. Double-Voting Checker
    # Check if this User OR this Device has already verified this report
    query = db.query(models.ReportVerification).filter(
        models.ReportVerification.report_id == report.id
    )
    if x_device_id:
        query = query.filter(
            (models.ReportVerification.user_id == current_user.id)
            | (models.ReportVerification.device_id == x_device_id)
        )
    else:
    ...
    if current_user.role == "coordinator" or current_user.role == "admin":
        ...
        # Log Verification
        new_verify = models.ReportVerification(
            report_id=report.id, user_id=current_user.id, device_id=x_device_id
        )
        db.add(new_verify)
    ...
    if not verify_data.lat or not verify_data.lng:
        raise HTTPException(
            status_code=400, detail="GPS location required for citizen verification"
        )
    ...
    if dist > max_radius:
        raise HTTPException(
            status_code=400,
            detail=f"You are too far away ({dist:.2f}km). Max allowed: {max_radius}km",
        )
...
def resolve_report(
    ...
    if current_user.role == "coordinator" or current_user.role == "admin":
        ...
        if (
            ...
            and current_user.state_assigned != report.state
        ):
            raise HTTPException(
                status_code=403,
                detail=f"You are only authorized for {current_user.state_assigned}",
            )
    ...
    if not resolve_data.lat or not resolve_data.lng:
        raise HTTPException(
            status_code=400, detail="GPS location required for citizen resolution"
        )

    dist = calculate_distance(
        resolve_data.lat, resolve_data.lng, report.lat, report.lng
    )
    ...
    if dist > max_radius:
        raise HTTPException(
            status_code=400,
            detail=f"Too far away ({dist:.2f}km). Max allowed: {max_radius}km",
        )
    ...
    if report.status == "verified":
        if not resolve_data.after_image_url:
            raise HTTPException(
                status_code=400, detail="Live photo required to report a fix"
            )
...
def get_system_settings(
    ...
    if not any(s.key == "proximity_radius_km" for s in settings):
        default_prox = models.SystemSetting(
            key="proximity_radius_km",
            value="0.5",
            description="Max radius (km) for verification",
        )
        db.add(default_prox)
        db.commit()
    ...
    if not any(s.key == "auto_post_x" for s in settings):
        default_auto = models.SystemSetting(
            key="auto_post_x",
            value="false",
            description="Auto-post verified reports to X",
        )
        db.add(default_auto)
        db.commit()
...
def update_system_setting(
    ...
    db_setting = (
        ...
        .filter(models.SystemSetting.key == setting.key)
        .first()
    )
    if not db_setting:
...
def get_proximity_radius(db: Session) -> float:
    setting = (
        db.query(models.SystemSetting)
        .filter(models.SystemSetting.key == "proximity_radius_km")
        .first()
    )
    if setting:
        try:
...
def get_auto_post_x(db: Session) -> bool:
    setting = (
        db.query(models.SystemSetting)
        .filter(models.SystemSetting.key == "auto_post_x")
        .first()
    )
    if setting:
...
def get_analytics(
    ...
    jur_counts = (
        db.query(models.Report.jurisdiction, func.count(models.Report.id))
        .group_by(models.Report.jurisdiction)
        .all()
    )
    ...
    return {"status": status_data, "distribution": jur_data, "growth": growth_data}
...
def get_admin_stats(
    ...
    pending_coordinators = (
        db.query(models.User)
        .filter(models.User.coordinator_application_status == "PENDING")
        .count()
    )

    recent_reports = (
        db.query(models.Report).order_by(models.Report.created_at.desc()).limit(5).all()
    )
